...
*/
        ...
        <div class="row mt-5">
            <div class="col-7 b-right">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb pl-0">
                        <li class="breadcrumb-item"><a href="{{route('cart.index')}}">Information</a></li>
                ...
                <div class="card mb-2">
                    <div class="col-12">
                        <div class="row mb-1 mt-1">
                            ...
                            <a class="btn btn-sm btn-primary-link ml-auto mr-2" href="{{route('profile')}}">Change</a>
                        </div>
                        <hr class="m-0">
                        <div class="row mb-1 mt-1">
                            <h5 class="contact-card mb-0 mt-2 ml-3">Ship To</h5>
                            <h5 class="email-card mb-0 mt-2 ml-3"> {{\Illuminate\Support\Str::limit($address->address, 20, $end="...")}}, {{$address->city}}, {{$address->country}} {{$address->zipcode}}</h5>
                            <a class="btn btn-sm btn-primary-link ml-auto mr-2" href="{{route('cart.shipping')}}">Change</a>
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('checkout.store') }}" method="POST" id="payment-form" enctype="multipart/form-data">
                    ...
                    {{-- <h2 class="shipping-head">Fedex Options</h2>--}}
                    {{-- <select name="quote" class="form-control ml-auto">--}}
                    {{-- @if(count($quotes) > 0)--}}
                    {{-- @foreach($quotes as $quote)--}}
                    {{-- <option value="{{$loop->iteration}}">${{$quote->RatedShipmentDetails->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount}} at {{$quote->DeliveryTimestamp}}</option>--}}
                    {{-- @endforeach--}}
                    {{-- @endif--}}
                    {{-- </select>--}}
                    ...
                    <label onclick="update_price({{$quote->RatedShipmentDetails->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount}})">
                        ...
                        <div class="card card-body bg-light d-flex flex-row justify-content-between align-items-center" style="color: black">
                            ...
                            ${{$quote->RatedShipmentDetails->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount}} at {{\Carbon\Carbon::parse($quote->DeliveryTimestamp)->toDayDateTimeString()}}
                        </div>
                    </label>
                    @endif
                    {{-- @if($quote->ServiceType == "STANDARD_OVERNIGHT")--}}
                    {{-- <label onclick="update_price({{$quote->RatedShipmentDetails->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount}})">--}}
                    {{-- <input required type="radio"  name="quote" class="card-input-element d-none" value="{{$loop->iteration}}" id="demo{{$loop->iteration}}">--}}
                    {{-- <div class="card card-body bg-light d-flex flex-row justify-content-between align-items-center" style="color: black">--}}
                    {{-- <span style="font-weight: bold">Overnight Shipping</span>--}}
                    {{-- ${{$quote->RatedShipmentDetails->ShipmentRateDetail->TotalNetChargeWithDutiesAndTaxes->Amount}} at {{\Carbon\Carbon::parse($quote->DeliveryTimestamp)->toDayDateTimeString()}}--}}
                    {{-- </div>--}}
                    {{-- </label>--}}
                    {{-- @endif--}}
                    ...
                    <div class="collapse" style="margin-bottom: 10rem" id="collapseExample">
                        ...
                        {{-- <div class="form-group">--}}
                        {{-- <input class="form-control input-address" name="ship_phone" placeholder="Phone">--}}
                        {{-- </div>--}}
                    </div>
                </form>
            </div>
            <div class="col-5">
                ...
                <div class="row ml-2 mb-3">
                    <div class="col p-0">
                        <h6 class="ml-2 text-primary" style="font-size: 15px; font-weight:600;">Estimated Arrival {{ $date_estimated}} </h6>
                    </div>
                </div>
                ...
                <div class="row ml-2 mb-2">
                    ...
                    <div class="col-10 p-0 mt-2">
                        ...
                        {{-- <div class="col-12">
                                    <div class="row">
                                        <div class="col-7">
                                            <h5 class="b-name">{{$item->model->user->vendor_profile->brand_name}}</h5>
                    </div>
    ...
    <div class="row ml-2 mt-1">
        ...
        <div class="col-2">
            <h5 class="price-summary ml-auto" style="white-space: nowrap;">${{Cart::instance('default')->tax()}}</h5>
        </div>
    </div>
    <hr>
    <div class="row ml-2">
        ...
        <div class="col-2">
            <h5 class="price-total ml-auto" id="cart_total">${{Cart::instance('default')->total()}}</h5>
        </div>
    </div>
    <div class="row">
        <div class="col-1">
    ...
    <script>
        (function() {
            ...
            var stripe = Stripe('pk_test_51HAVtEHT5fE06PMkM5d8mhS4fppSU8RcU3NPdtCeXRBXTMWRYeWua4ea9bM0mnsTLYQcCCs9Kyy08EdWl78viinl00cQt1howv');
