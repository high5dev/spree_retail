                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <form id="create" action="{{route('vendor.dashboard.product.create')}}" method="GET"></form>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                @if(count($products) > 0)
                                    @foreach($products as $product)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('vendor.dashboard.product.edit',$product->slug)}}" style="color: blue" class="btn">
                                                    Edit
                                                </a>
                                                <a href="" onclick="event.preventDefault();document.getElementById('delete-{{$product->id}}').submit()" style="color: red" class="btn">
                                                    Delete
                                                </a>
                                                <form id="delete-{{$product->id}}" action="{{route('vendor.dashboard.product.destroy',$product->slug)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
