...
@section('content')
            ...
            <div class="col-2">
                <div class="filterContainer">
                    <div class="firstInFilter">
                        <h1 style="white-space: nowrap;">{{ $title ?? ($featured ?? 'New Arrivals') }}</h1>
                    ...
                    <div class="filters">
                        ...
                        <div class="collapse" id="collapseExample-category">
                            @foreach($categories as $key => $category)
                                <div class="filters">
                                    <a style="color: black;cursor: pointer" onClick="filterByOption('{{$category}}')"  class="toggle__text text-gray garage-title">{{$category}}</a>
                                </div>
                            @endforeach
                        </div>
                        <div class="underline"></div>
                            ...
                            <div >
                                @if(count($sizes) > 0)
                                    @foreach($sizes as $size)
                                        <div style="margin-top:12px"><input type="checkbox" name="s[]" class="checkbox-price" onChange="filterByOption()" value="{{$size->id}}" style="display: inline;
    vertical-align: middle;">
    ...
    -moz-user-select: none;
    -ms-user-select: none;
    user-select: none;">{{ $size->name }}</div>
                                        </div>
                                    @endforeach
                                @endif
                            </div>
    ...
    line-height: 24px;
    margin-left: 8px;">{{$color->name}}</span>
                                    </div>
                                @endforeach
                            @endif
                        </div>
