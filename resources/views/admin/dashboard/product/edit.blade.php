                    ...
                    <form action="{{route('product.update',$product->slug)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="featured_category" class="form-control" id="featured">
                                ...
                                @foreach($featured_cats as $featured_cat)
                                    @if($product->featured == $featured_cat->name)
                                        <option selected value="{{$featured_cat->name}}">{{$featured_cat->name}}</option>
                                    @else
                                        <option value="{{$featured_cat->name}}">{{$featured_cat->name}}</option>
                                    @endif
                                @endforeach
                            </select>
                            @error('main_category')
                        ...
                        <div class="form-group">
                            ...
                            <select name="parent_category" class="form-control" id="parent" onChange="parentSubSelected(this);">
                                <option>Select</option>
                                @if(count($parents) > 0 AND  $parent_cat != null)
                                    @foreach($parents as $item)
                                        @if($item->name == ($parent_cat != null ? $parent_cat->name : "NULL"))
                            ...
                            <select name="parent_sub_category" class="form-control" id="sub-parent">
                                ...
                                @if(count($childs) > 0)
                                    @foreach($childs as $item)
                                        @if($item->name == ($sub_cat != null ? $sub_cat->name : "NULL"))
                                            <option selected value="{{$item->name}}">{{$item->name}}</option>
                                        @else
                                            <option value="{{$item->name}}">{{$item->name}}</option>
                                        @endif
                            ...
                            <select  onChange="typeSizeSelected(this);"  name="type_size" class="form-control" id="type_size">
                                ...
                                @if(count($type_sizes) > 0)
                                    @foreach($type_sizes as $item)
                                        @if($item->id == $product->type_size_id)
                        ...
                        <div class="custom-file mb-3 col-4">
                            <input type="file" name="thumbnail[]" multiple class="custom-file-input @error('thumbnail') is-invalid @enderror" id="customFile">
