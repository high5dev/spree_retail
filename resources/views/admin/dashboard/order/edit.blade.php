    ...
    <div class="dashboard-wrapper">
        ...
        <div class="container">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.dashboard.user.update', $user->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="first_name" class="col-form-label">First Name</label>
                            <input id="inputText3" value="{{old('first_name') ?? $user->first_name}}" name="first_name" type="text" class="form-control @error('first_name') is-invalid @enderror">
                            @error('first_name')
                        ...
                        <div class="form-group">
                            ...
                            <input id="inputText3" value="{{old('last_name') ?? $user->last_name}}" name="last_name" type="text" class="form-control @error('last_name') is-invalid @enderror">
                            @error('last_name')
                            <div class="invalid-feedback">
                                <strong>{{ $message }}</strong>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input id="email" name="email" value="{{old('email') ?? $user->email}}" type="email" placeholder="Email" class="form-control @error('email') is-invalid @enderror">
                            @error('email')
                            <div class="invalid-feedback">
                                <strong>{{ $message }}</strong>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" class="form-control" id="input-select">
                                ...
                                @foreach(config('enums.user_role') as $role)
                                    @if($role == $user->roles[0]->name)
                                        <option selected value="{{$loop->iteration}}">{{$role}}</option>
                                    @else
                                        <option value="{{$loop->iteration}}">{{$role}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control" id="input-select">
                                <option value="">Select</option>
                                @foreach(config('enums.user_status') as $status)
                                    @if($status == $user->status)
                                        <option selected value="{{$loop->iteration}}">{{$status}}</option>
                                    @else
                                        <option value="{{$loop->iteration}}">{{$status}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
