                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex">
                            ...
                            <form id="create" action="{{route('admin.dashboard.category.create')}}" method="GET"></form>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table">
                                ...
                                @if(count($categories) > 0)
                                    @foreach($categories as $category)
                                        <tr>
                                            <td>{{$category->name}}</td>
                                            <td>{{count($category->parents) > 0 ? implode(', ', $category->parents->pluck('name')->toArray()) : 'None'}}</td>
                                            <td>{{$category->created_at}}</td>
                                            <td>
                                                <a href="{{route('admin.dashboard.category.show',$category->slug)}}" style="color: blue" class="btn">
                                                    Show Products
                                                </a>
                                                <a href="{{route('admin.dashboard.category.edit',$category->slug)}}" style="color: blue" class="btn">
                                                    Edit
                                                </a>
                                                <a href="" onclick="event.preventDefault();document.getElementById('delete-{{$category->id}}').submit()"  style="color: red" class="btn">
                                                    Delete
                                                </a>
                                                <form id="delete-{{$category->id}}" action="{{route('category.destroy',$category->slug)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
